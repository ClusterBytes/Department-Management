...
def delete_faculty(request, f_id):
    ...
    if login_data.is_hod:
        messages.error(
            request,
            "Cannot delete your account " + f_data.First_name + f_data.Last_name,
        )
        return redirect(view_faculty)
    else:
        f_data.delete()
        ...
        messages.error(
            request,
            "Successfully deleted the faculty " + f_data.First_name + f_data.Last_name,
        )
        return redirect(view_faculty)
...
def create_scheme(request):
    ...
    if request.method == "POST":
        ...
        if scheme_count:
            messages.error(request, "Already exist " + scheme_input)
            return redirect(create_scheme)
        else:
            scheme.objects.create(scheme=scheme_input)
            messages.error(request, "Successfully created " + scheme_input)
            return redirect(create_scheme)
...
def create_subject(request):
    ...
    if "create_subject" in request.POST:
        ...
        subject_exist = subject.objects.filter(
            code=subject_code, scheme=scheme_id_int
        ).count()
        if subject_exist == 0:
...
def edit_subject(request, subject_id):
    ...
    if "edit_subject" in request.POST:
        ...
        if subject_exist == 0:
            ...
            messages.error(
                request, "The Subject " + subject_name + " successfully Updated"
            )
    ...
    return render(
        ...
        "edit_subject.html",
        {
            "context": context,
            "scheme_data": scheme_data,
            "previous_subject_data": previous_subject_data,
            "previous_scheme_data": previous_scheme_data,
            "data_for_self_profile": staff_details_1,
        },
    )

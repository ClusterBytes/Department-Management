...
def view_subjects(request):
    ...
    assigned_subject_to_this_staff = subject_to_staff.objects.filter(
...
def update_class(request, batch_id, subject_id):
    ...
    batch_data = batch.objects.all()
    staff_data = profile.objects.all()
    att_record = attendance_record.objects.filter(
        batch_id=batch_id, subject_id=subject_id
    )
    subject_to_staff_data = subject_to_staff.objects.filter(
        batch_id=batch_id, subject_id=subject_id
    )
    ...
    check_today_attendance_marked = attendance_record.objects.filter(
        batch_id=batch_id, subject_id=subject_id, date=today
    )
...
def add_attendance(request, batch_id, subject_id):
    ...
    if request.method == "POST":
        ...
        att_record = attendance_record.objects.filter(
            date=today, from_time=from_time, subject_id=subject_id, batch_id=batch_id
        )
        ...
        if att_record:
            ...
            record_id = attendance_record.objects.get(
                date=today,
                marked=True,
                subject_id=subject_id,
                batch_id=batch_id,
            ...
            for i in student_data:
                ...
                attendance.objects.create(
                    attendance_record_id=record_id.id,
                    student_id=i.id,
                    subject_id=subject_id,
                    batch_id=batch_id,
                    present=att_mark,
        ...
        for j in mark_data:
            if i.id == j.student_id:
                sum_of_mark = Internal_mark.objects.filter(
                    student_id=i.id, subject_id=subject_id
                ).aggregate(Sum("mark"))
...
def view_classes(request):
    ...
    return render(
        request,
        "view_classes.html",
        {"context": context, "batch_data": batch_data, "scheme_data": scheme_data},
    )
...
def university_result(request, batch_id):
    ...
    if "semester" in request.POST:
        ...
        subject_this_sem = subject_to_staff.objects.filter(
            semester=sem, batch_id=batch_id
        )

        return render(
            request,
            "university_result.html",
            {
                "context": context,
                "student_data": student_data,
                "subject_data": subject_data,
...
def update_student_profile(request, student_id):
    ...
    for i in assign_subject_data:
        ...
        sum_of_mark = Internal_mark.objects.filter(
            student_id=id, subject_id=i.subject_id
        ).aggregate(Sum("mark"))
        total_internal = sum_of_mark["mark__sum"]
...
def add_result(request, batch_id):
    ...
    return render(request, "add_result.html", {"context": context})
...
def add_sem_result(request, batch_id, month, year, semester):
    ...
    student_data = profile_student.objects.filter(batch=batch_id)
    subject_in_sem = subject_to_staff.objects.filter(
        batch_id=batch_id, semester=semester
    )
    all_subject = subject.objects.all()
    ...
    if request.method == "POST":
        ...
        for j in student_data:
            for i in subject_in_sem:
                ...
                month_int = month
                year_int = year
                already_exist = semester_result.objects.filter(
                    university_no=j.university_no,
                    subject_id=i.subject_id,
                ...
                if already_exist:
                    ...
                    chance_count = semester_result.objects.filter(
                        university_no=j.university_no, subject_id=i.subject_id
                    ).count()
                    if chance_count == 0:
                        semester_result.objects.create(
                            university_no=j.university_no,
                            subject_id=i.subject_id,
                            grade_point=float(x[0]),
                            semester=semester,
...
def report(request, batch_id, semester):
    ...
    subject_data = subject.objects.all()
    subject_in_sem_id = subject_to_staff.objects.filter(
        semester=semester, batch_id=batch_id
    )
    ...
    for i in student_data:
        ...
        tuple_data = (
            i.university_no,
            arrears_in_current_sem,
            absent,
            round(sgpa / total_credit, 2),
        )
    ...
    for i in range(1, 5):
        for j in student_data:
            ...
            for subject_in_sem in subject_in_sem:
                if (
                    semester_result.objects.filter(
                        subject_id=subject_in_sem.subject_id,
                        semester=i,
                        university_no=j.university_no,
                    ).count()
                    > 1
                ):
                    chance = semester_result.objects.filter(
                        subject_id=subject_in_sem.subject_id,
                        semester=i,
                        university_no=j.university_no,
                    ).count()
                    supply = semester_result.objects.filter(
                        subject_id=subject_in_sem.subject_id,
                        semester=i,
                        university_no=j.university_no,
                        no_of_chances=chance,
                        grade_point__lte=0,
                    )
                    if supply:
                        count += 1
    ...
    for i in subject_in_sem_id:
        for j in subject_data:
            if i.subject_id == j.id:
                no_of_absent = semester_result.objects.filter(
                    subject_id=j.id, no_of_chances=1, semester=semester, grade_point=-1
                ).count()
    ...
    for i in subject_in_sem_id:
        for j in subject_data:
            if i.subject_id == j.id:
                no_of_passed = semester_result.objects.filter(
                    subject_id=j.id,
                    no_of_chances=1,
                    semester=semester,
                    grade_point__gte=5,
    ...
    for i in subject_in_sem_id:
        for j in subject_data:
            if i.subject_id == j.id:
                no_of_failed = semester_result.objects.filter(
                    subject_id=j.id,
                    no_of_chances=1,
                    semester=semester,
                    grade_point__lte=0,
    ...
    for i in subject_in_sem_id:
        for j in subject_data:
            if i.subject_id == j.id:
                no_of_o_gr = semester_result.objects.filter(
                    subject_id=j.id, no_of_chances=1, semester=semester, grade_point=10
                ).count()
...
def performance_analysis(request, batch_id):
    ...
    if request.method == "POST":
        ...
        # date_dob = str(date_of_birth)  # dob can only display in html only as string type
        assign_subject_data = subject_to_staff.objects.filter(
            batch_id=batch_id, semester=sem
        )
        internal_mark_data = Internal_mark.objects.filter(student_id=student_id)
        subject_data = subject.objects.all()
        for i in internal_mark_data:
